    ...
    animPromise.then(resolve => {
            if (anim.currentFrame !== 0 && updateAnimation) {
                ...
                if (anim.isPaused && isOn) {
                    anim.playSegments([lineup.bate[current_bat].in, lineup.bate[current_bat].out], true);
                    ...
                    if (
                        ...
                        if (animElement.data.hasOwnProperty('refId') && animElement.data.refId.includes('image')) {
                            ...
                            try {
                                ...
                                if (animElement.data.hasOwnProperty('color')){
                                    console.log(`numeros Color: ${animElement.data.nm}`);
                                     animElement.updateDocumentData({
                                 ...
                                 //    alert("I am an alert box: animElement.data.lineup == current_bat");
                                     console.log(`Lineup Color texto barra: ${animElement.data.nm} lineup:${animElement.data.lineup}, current_bat: ${current_bat}`)
                                     animElement.updateDocumentData({
                                             t: data[cl] ? data[cl].text || data[cl] : '', fc: data_equipos[current_team].color_texto}, 0); // Update the text y coloreamos blaco     
                                 }else if(animElement.data.hasOwnProperty('lineup') && animElement.data.lineup !== current_bat){
                                        //    alert("I am an alert box: animElement.data.lineup == current_bat");
                                            console.log(`Lineup lineup Negro: ${animElement.data.nm} lineup:${animElement.data.lineup}, current_bat: ${current_bat}`)
                                            animElement.updateDocumentData({
                                                    t: data[cl] ? data[cl].text || data[cl] : '', fc: [0,0,0]}, 0); // Update the text y coloreamos negro     
                                            }
...
function albate(value){
    anim.firstFrame = 0;
    console.log("albate: " + value )
    console.log("value: " + lineup.bate[value].out)
    anim.goToAndStop(lineup.bate[value].out,true);
  //  anim.playSegments([lineup.bate[value].in, lineup.bate[value].out], true);
}
